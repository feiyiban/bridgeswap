...
import (
	...
	xfsgocrypto "bridgeswap/sdk/xfsgo/crypto"
...
func (k *Key) UnmarshalJSON(j []byte) (err error) {
	...
	*u = uuid.Parse(keyJSON.ID)
	...
	k.Address = common.Bytes2Address(addr)
...
func newKeyFromECDSA(privateKeyECDSA *ecdsa.PrivateKey) *Key {
	...
	key := &Key{
		ID:      id,
		Address: xfsgocrypto.DefaultPubKey2Addr(privateKeyECDSA.PublicKey),
		// Address:    crypto.DefaultPubKey2Addr(privateKeyECDSA.PublicKey),
...
func keyFileName(keyAddr common.Address) string {
	ts := time.Now().UTC()
	return fmt.Sprintf("UTC--%s--%s", toISO8601(ts), keyAddr.B58String())
}
